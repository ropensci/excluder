...
The verbs and exclusion types combine with `_` to create the functions, such as `check_duplicates`, `mark_duration`, and `exclude_ip`. Multiple functions can be changed together using the [`{magrittr}`]() pipe `%>%`. For datasets downloaded directly from Qualtrics, use `remove_label_rows()` to remove the first two rows of labels and convert date and numeric columns in the metadata.
